# -*- coding: utf-8 -*-
"""대량보유상황보고서(holding) DSD-XML → 마크다운 변환기.

대상: corpus/raw/holding/**/*.xml  (1,083건)
major_parser.py 와 같은 골격(관대한 HTML 파서, 셀 모델, 행 분류 알고리즘)을
그대로 쓰되, holding 원문을 전수 훑어 확인한 사실에 맞게 두 가지를 더했다.
(아래 "major_parser.py 와 다른 점" 참조)

표준 라이브러리만 쓴다. 설치할 게 없다.

────────────────────────────────────────────────────────────────────────
원본 1,083건을 전부 훑어서 확인한 사실
────────────────────────────────────────────────────────────────────────
 · 인코딩  : 1,083/1,083 UTF-8. cp949 폴백은 방어용.
 · 태그    : major와 같은 DSD-XML 스키마(TD/TH/TE/TU, SECTION-1~4, TABLE-GROUP …).
 · 정정    : <CORRECTION> 태그 유무 = manifest의 is_correction. 41건.
 · 중첩표  : 5건뿐 (major는 126건). 드물지만 있으므로 그대로 처리한다.
 · 문서종류 2종류: 대량보유상황보고서(일반) 607 / (약식) 476.

┌ 표 4분류 (33,011개) ───────────────────────────────────────────────────┐
│ ① THEAD 있음     12,147 (37%)  → 표 그대로 유지 — holding의 주류        │
│ ② TE/TU 있음      9,790 (30%)  → - **항목**: 값                        │
│ ③ 1행 1칸         6,437 (19%)  → 제목/문단                             │
│ ④ TD만            4,637 (14%)  → 표지·목록·일부 재무 표                │
└───────────────────────────────────────────────────────────────────────┘

 · rowspan 최대 123, rowspan+colspan 동시 2,221군데.

────────────────────────────────────────────────────────────────────────
major_parser.py 와 다른 점 (둘 다 원문 전수 확인 후 반영)
────────────────────────────────────────────────────────────────────────
 ① 머리글 없는 TD표(④, 4,637개)에도 진짜 머리글이 있는 경우가 있다.
    THEAD가 아니라 배경 음영(USERMARK 안 "BC0X..." 코드)으로만 머리글을
    표시하는 표가 섞여 있다 — periodic의 재무제표 주석 표와 같은 관행이다.
    이 신호는 TD에만 붙고 TE/TU에는 전혀 안 붙는다(전수 확인).
    놓치면 "구분1/구분2/…" 같은 가짜 머리글로 실제 항목명이 사라진다.
    → _flush_buffer() 가 버퍼 맨 위 몇 줄이 전부 머리글 표시(TH 또는
      음영 TD)인지 먼저 보고, 있으면 그 줄들을 합쳐 진짜 머리글로 쓴다.
...
__all__ = ['parse_file', 'parse', 'to_markdown', 'to_dict', 'decode',
           'HoldingParser', 'Stats', 'corp_name_from_path',
           'looks_like_holding_xml']
...
class HoldingParser:
    """holding XML 하나를 조각(chunk) 목록으로 바꾼다.
...
def to_markdown(doc, with_header=True):
    ...
    """
    return _render_dsd(doc, '대량보유상황보고서', _iso, _esc, with_header)
...
def to_dict(doc):
    ...
    return {
        '회사명': doc['회사명'],
        # <COMPANY-NAME> 원문값 (holding에서는 발행회사 등기명).
...
def corp_name_from_path(path):
    """raw/holding/<법인명>/<접수번호>/<접수번호>.xml 에서 법인명을 뽑는다."""
    import os
    return os.path.basename(os.path.dirname(os.path.dirname(path)))


def looks_like_holding_xml(source):
    return '<DOCUMENT' in source[:600].upper()


def parse(source, receipt_no=None, corp_name=None, **kw):
    return HoldingParser(**kw).parse(source, receipt_no, corp_name)
