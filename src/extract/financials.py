...
# 라벨에 붙는 **표시 장식**. 항목 이름이 아니다.
#   Ⅵ.기말현금및현금성자산   VI.기초의현금및현금성자산   1.영업활동현금흐름
#   기타금융자산(주5,27)      재고자산(주7)
# 서식마다 붙였다 뗐다 하므로 후보를 무한히 늘리는 대신 여기서 벗긴다.
# 실측: 이 둘 때문에 CF·BS 의 좌변 라벨을 못 찾아 검사가 통째로 건너뛰었다.
_RE_LEAD_NUM = re.compile(
    r'^(?:[ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩIVXivx]{1,5}|[0-9]{1,2}|[가-힣])\s*[.)]\s*')
_RE_NOTE_REF = re.compile(r'\(\s*주\s*\d[^)]*\)\s*$')


def _norm_label(s):
    t = re.sub(r'\s+', '', s or '')
    t = _RE_NOTE_REF.sub('', t)
    # 접두는 겹칠 수 있다 (예: 'Ⅵ.1.기말현금')
    for _ in range(3):
        t2 = _RE_LEAD_NUM.sub('', t)
        if t2 == t:
            break
        t = t2
    return t
...
_CF_FX = ['현금및현금성자산에대한환율변동효과', '외화표시현금및현금성자산의환율변동효과',
          '현금및현금성자산의환율변동효과']
# 보고 주기에 따라 '기말' 이 '분기말'·'반기말' 이 된다. 실측으로 확인했다.
_CF_BEG = ['기초현금및현금성자산', '기초의현금및현금성자산',
           '기초의현금및현금성자산잔액', '현금및현금성자산의기초잔액',
           '기초의현금및현금성자산', '기초현금및현금성자산잔액']
_CF_END = ['기말현금및현금성자산', '기말의현금및현금성자산',
           '기말의현금및현금성자산잔액', '현금및현금성자산의기말잔액',
           '분기말현금및현금성자산', '분기말의현금및현금성자산',
           '반기말현금및현금성자산', '반기말의현금및현금성자산',
           '당기말현금및현금성자산', '당기말의현금및현금성자산']
...
LINEAR_CHECKS = {
    ...
    'IS': [
        ...
        # 기타포괄손익 = 재분류되지 않는 항목 + 재분류되는 항목.
        # 손익계산서 별표(IS2/IS3)에서 가장 흔한 구조다 — 실측 433개 그룹이
        # 이 두 항목을 갖고 있는데 매출 기반 규칙은 하나도 안 맞는다.
        ('기타포괄손익 분해', _OCI,
         [(1, ['후속적으로당기손익으로재분류되지않는항목',
          ...
          (1, ['후속적으로당기손익으로재분류될수있는항목',
                ...
                '당기손익으로재분류될수있는항목',
                '후속기간에당기손익으로재분류되는항목',
                '당기손익으로재분류될수있는항목(세후기타포괄'])], []),
        # 금융업·지주사 손익계산서는 매출총이익 대신 영업외손익을 쓴다.
        ('세전이익', _PRETAX,
         [(1, ['영업이익', '영업이익(손실)']),
          (1, ['영업외손익', '영업외수익', '기타영업외손익'])], []),
        ('총포괄손익 귀속', _TCI,
...
def _equity_column_checks(group, tolerance=1):
    ...
    **별도재무제표에는 그 열이 없다** — 자회사가 없으니 지배/비지배 구분이
    ...
    for r in rows:
        ...
        # 구성요소가 **전부** 비었는데 합계에만 값이 있으면, 그 값은 내가
        # 합계 열로 분류해 제외한 중간 소계 쪽에 들어 있다는 뜻이다.
        # 검사할 근거가 없으므로 건너뛴다 — 0 으로 채워 더하면 상대오차가
        # 정확히 1.0 인 거짓 불일치가 되고(실측 480여 건), 멀쩡한 표가
        # low 로 강등돼 facts_* 진입이 막힌다.
        if all(v is None for v in vs):
            continue
        # 일부만 빈 칸은 0 으로 본다 — 자본변동표는 해당 없는 칸을 비워 둔다.
