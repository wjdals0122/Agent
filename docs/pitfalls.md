...
| 항목 | 상태 |
|---|---|
| PDF 문서 3건 | KB금융·한화오션·한화에어로스페이스. XML이 없어(`file_format: pdf+html`) 파싱 대상에서 빠짐. `no_source_xml`로 기록됨 |
| 비교 질의 | "삼성전자와 SK하이닉스 매출액 비교" → 상위 20건이 전부 한쪽 회사. 질의 벡터가 두 개체의 혼합이라 승자독식 |
| 답변 생성 | LLM 미연결. 검색 근거까지만 |
| 청커 2종 병존 | 배포본(61만, 임베딩됨) vs 레포 06단계(129만, 미임베딩). `meta.json`의 `chunks_file_sha1`이 사후 추적은 해준다 |
