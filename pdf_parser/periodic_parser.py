...
def decode(raw_bytes):
    """periodic 1,466개는 전부 UTF-8이다. cp949 폴백은 방어용."""
...
def clean(s):
    ...
    lines = [_RE_MULTISPACE.sub(' ', l).strip() for l in s.split('\n')]
    return '\n'.join(l for l in lines if l)


def flat(s):
    """한 줄로 눌러 담는다 (표 셀, 제목용)."""
    return _RE_MULTISPACE.sub(' ', clean(s).replace('\n', ' ')).strip()
...
def _text(node):
    out = []
    for r in node.raw:
        out.append(_text(r) if isinstance(r, _Node) else r)
    return ''.join(out)
...
def _own_nodes(node):
    ...
    for c in node.children:
        if c.tag == 'TABLE':
            continue
        yield c
        for g in _own_nodes(c):
            yield g


def _own_tables(node):
...
_RE_COLON_LABEL = re.compile(r'^(.{1,30}?)\s*[:：]\s*$')

_SECTION_TAGS = {'SECTION-1': 2, 'SECTION-2': 3, 'SECTION-3': 4, 'SECTION-4': 5}
_IGNORE = {'PGBRK', 'COLGROUP', 'COL', 'SUMMARY', 'EXTRACTION',
...
class PeriodicParser:
    ...
    def _render_row(self, grid_row, ncol):
        ...
        for c in range(ncol):
            ...
            out.append(cell.text if (cell and cell is not prev) else '')
            prev = cell
        return out
    ...
    def _flush_buffer(self, grid, ncol, buf):
        ...
        rows = [[row[c].text if c < len(row) and row[c] else ''
        ...
        while i < len(runs):
            ...
            if kind == 'L' and nxt and nxt[0] == 'V':
                ...
                if after is not None and after[0] == 'L' and i + 3 >= len(runs):
                    ...
                    consumed = 3
                for v in nxt[1]:
                    out.extend(self._emit(ctx, labels, v))
                matched = True
                i += consumed
...
_RE_ISO8 = re.compile(r'^(\d{4})(\d{2})(\d{2})$')
_RE_ISO_RANGE = re.compile(r'^(\d{8})-(\d{8})$')


def _iso(norm):
    if not norm:
        return None
    m = _RE_ISO8.match(norm)
    if m:
        return '%s-%s-%s' % m.groups()
    m = _RE_ISO_RANGE.match(norm)
    if m:
...
def to_markdown(doc, with_header=True):
    ...
    if with_header:
        lines.append('# %s' % (doc['문서종류'] or '사업보고서'))
        meta = [x for x in [
            doc['회사명'],
    ...
    for c in doc['chunks']:
        ...
        if kind == 'h':
            ...
            if value is None:
                ...
                continue
            iso = _iso(norm)
            tail = ' (%s)' % iso if (iso and iso not in value) else ''
            if '\n' in value:
                seg = value.split('\n')
            ...
            for r in rows:
                cells = [_esc(x) for x in r] + [''] * (n - len(r))
